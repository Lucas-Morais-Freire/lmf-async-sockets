...
Tarefa<ReturnT>::AwaiterFinal::AwaiterFinal(std::coroutine_handle<PromiseBase> crth_mae) noexcept :
_crth_mae{crth_mae} {}



template <typename ReturnT>
std::coroutine_handle<PromiseBase> Tarefa<ReturnT>::AwaiterFinal::await_suspend(std::coroutine_handle<Promise<ReturnT>> crth) const noexcept {
  // Fazer cópias locais pois o ponteiro `this` pode ser invalidado após enfileirarmos `crth` ou `_crth_mae`.
  std::coroutine_handle<PromiseBase> crth_mae    = _crth_mae;
  Escalonador                       *escalonador = crth.promise().escalonador();
  ...
  if (prox_crth == std::noop_coroutine()) {
    ...
    escalonador->enfileirar(crth.promise().crth());
  ...
  // e nos consumir
  if (crth_mae) escalonador->enfileirar(crth_mae.promise().crth());
